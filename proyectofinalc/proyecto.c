...
void cargarProductos(const char *filename, Producto **productos, int *n) {
    ...
    if (!file) {
        printf("Archivo no encontrado. Se iniciará con inventario vacio.\n");
        *productos = NULL;
        *n = 0;
        return;
    }
...
void modificarProducto(Producto *productos, int n) {
    ...
    int index;
    printf("Ingrese el numero del producto a modificar: ");
    scanf("%d", &index);
    if (index < 1 || index > n) {
...
int main() {
    ...
    cargarProductos(archivoEntrada, &inventario, &cantidad);
    ...
    do {
        ...
        switch (opcion) {
            case 1: mostrarProductos(inventario, cantidad); break;
            case 2: agregarProducto(&inventario, &cantidad); break;
            case 3: modificarProducto(inventario, cantidad); break;
            case 4: eliminarProducto(&inventario, &cantidad); break;
            case 5:
                guardarProductos(archivoSalida, inventario, cantidad);
